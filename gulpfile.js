...
gulp.task('semantic-asset', function () {
    return mergeStream.apply(null, globs.semantic.map(function(glob) {
        return gulp.src(glob)
            .pipe(gulp.dest(
                glob.replace(/\/\*.*$/, '')
                    .replace(/^semantic\/dist/, 'public/css')
            ));
    }));
});
//gulp.task('semantic-build', []);
gulp.task('semantic-build', require('./semantic/tasks/build'));
//gulp.task('semantic', ['semantic-build', 'semantic-asset']);
